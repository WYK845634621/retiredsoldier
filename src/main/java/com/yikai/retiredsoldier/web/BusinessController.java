...
public class BusinessController {
    ...
    /**
     * @Note: 导出Excel
     * @Date:2019/3/24 13:30 @Auth:yikai.wang @Desc(V/B):〈〉
     */
    @GetMapping("/exportBusinessExcel")
    public void exportBusinessExcel(HttpServletRequest request, HttpServletResponse response){
        try {
            List<Business> businesses = businessService.selectList(null);
            ExcelUtil.generateExcel(response,businesses);

        }catch (Exception e){
            logger.error("error in BusinessController.exportBusinessExcel", e);
        }
    }
